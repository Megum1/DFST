...
if __name__ == '__main__':
    ...
    model_file = 'vgg_denoise_sunrise_1'
    ...
    for hl_idx in range(len(model.layers)):
        if 'conv' in model.layers[hl_idx].name:
            ...
            flog = open('max_calue.txt', 'a')
            flog.write('Layer: {0} Value: {1}\n'.format(layer_name, max_value))
            ...
            for idx in range(n_neurons):
                ...
                if dif > 5 * max_value and dif > Beni_value:
                    ...
                    flog.write('Layer: {0} Neuron: {1} Trojan_value: {2} Benign_value: {3}\n'.format(layer_name,
                                                                                                     ...
                                                                                                     Beni_value))
                    flog.close()
                    comp_neurons[layer_name].append(idx)
            '''
    # print(comp_neurons)
